...
export default function DestructionPage() {
  ...
  const onDestroy = async () => {
    try {
      ...
      setConfirmText('');
      if (response.skipped) {
        message.warning('다른 인스턴스에서 백업/파기 작업이 진행 중입니다. 잠시 후 다시 시도해 주세요.');
      } else {
        message.success('휴폐업 일괄 파기가 완료되었습니다.');
      }
    } catch (error) {
      message.error(error instanceof Error ? error.message : '휴폐업 파기에 실패했습니다.');
    }
  };

  return (
    <div className='max-w-3xl'>
      <PageHeader title='휴폐업 일괄 파기' description='보존 목적이 종료된 위치정보를 백업 후 일괄 파기합니다.' />
      ...
      <Alert
        ...
        className='mb-5'
        message='위험 작업'
        description='이 작업은 되돌릴 수 없습니다. 대상 데이터는 이름·연락처를 제거하여 백업(HISTORY)한 뒤 원장에서 삭제됩니다.'
      />
      ...
      <div className='mb-6 rounded border border-gray-300 bg-white p-5'>
        ...
        {isFetching ? (
          ...
          <Descriptions column={2} bordered size='small'>
            ...
            <Descriptions.Item label='백업 건수'>{formatNumber(status?.historyCount)}</Descriptions.Item>
      ...
      {result ? (
        result.skipped ? (
          <Result
            ...
            status='warning'
            title='작업이 스킵되었습니다'
            subTitle='다른 인스턴스에서 백업/파기 작업이 진행 중입니다. 잠시 후 다시 시도해 주세요.'
          />
        ) : (
          <Result
            className='mt-6'
            status='success'
            title='휴폐업 파기 완료'
            subTitle={`백업 ${formatNumber(result.backedUp)}건 · 삭제 ${formatNumber(result.deleted)}건 처리되었습니다.`}
          />
        )
      ) : null}
    </div>
